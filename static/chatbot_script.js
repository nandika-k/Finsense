...
class ChatbotUI {
    ...
    async initializeAuth() {
        ...
        if (FLASK_AUTH_MODE && FLASK_USER) {
            ...
            // Load chat history for Flask mode
    ...
    async loadChatHistory() {
        // Skip if not authenticated, or if no token and not Flask mode
        if (!this.isAuthenticated || (!this.accessToken && !FLASK_AUTH_MODE)) {
            return;
        }
    ...
    getHeaders() {
        // Flask auth mode uses X-User-ID header instead of Bearer token
        if (FLASK_AUTH_MODE && this.userId) {
            return { 
                'Content-Type': 'application/json',
    ...
    formatMarkdown(text) {
        ...
        // Match URLs in parentheses like: (https://example.com)
