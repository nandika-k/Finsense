...
def parse_with_llm(
    client: Any, user_input: str, context: str, valid_options: List[str]
) -> Optional[List[str]]:
    """
...
def collect_investment_goals() -> List[str]:
    ...
    while True:
        ...
        try:
            ...
            print(
                "  [!] Could not parse input. Try numbers (e.g., '1,3') or describe your goals."
            )
...
def collect_sector_preferences(suggested_sectors: List[str]) -> List[str]:
    ...
    while True:
        ...
        if llm_client and is_delegating_decision(
            llm_client, response, "sector selection"
        ):
            if suggested_sectors:
                print(
                    "  -> Great! I'll use the suggested sectors based on your goals:"
                )
                print(f"     {', '.join(suggested_sectors)}")
                return suggested_sectors
            else:
                ...
                print("  -> I'll select a diversified mix of sectors:")
        ...
        if suggested_sectors and any(
            keyword in response_lower
            for keyword in ["suggested", "suggest", "recommendation"]
        ):
            ...
            if llm_client:
                ...
                if additional_text:
                    ...
                    additional_sectors = parse_sectors_with_llm(
                        llm_client, additional_text
                    )

                    if additional_sectors:
                        # Remove sectors already in suggested
                        additional_sectors = [
                            s for s in additional_sectors if s not in suggested_sectors
                        ]
        ...
        if response[0].isdigit() or "," in response:
            try:
                ...
                print(
                    f"  [!] Could not parse numbers. Valid range: 1-{len(AVAILABLE_SECTORS)}"
                )
                continue
...
def confirm_preferences(
    goals: List[str], sectors: List[str], risk_tolerance: str
) -> bool:
...
def run_chatbot() -> Optional[Dict]:
    ...
    while True:
        ...
        if initial_input:
            ...
            if any(
                keyword in initial_lower
                for keyword in ["example", "sample", "show me example", "demo"]
            ):
                print("\n" + "=" * 80)
...
Answer (true or false):"""
                        ...
                        asking_what_needed = (
                            resp2.choices[0].message.content.strip().lower() == "true"
                        )
                except Exception:
                    # Fallback to keyword matching if LLM fails
                    asking_for_sector_help = any(
                        keyword in initial_lower
                        for keyword in [
                            "what sectors",
                            "show sectors",
        ...
        if initial_input:
            ...
            if understood:
                ...
                if shown_understood and initial_lower in [
                    "yes",
                    "y",
                    "yep",
                    "yeah",
        ...
        if goals is None:
            ...
            while True:
                ...
                if result is True:
                    ...
                    print(
                        "  [!] I'm not sure what you mean. Please answer 'yes' or 'no'."
                    )
