...
INVESTMENT_GOALS = ["growth", "income", "esg", "value", "defensive", "diversified"]
...
class UserPreferences:
    ...
    def validate(self) -> List[str]:
        ...
        if invalid_sectors:
            errors.append(
                f"Invalid sectors: {invalid_sectors}. Valid: {AVAILABLE_SECTORS}"
            )

        # Validate risk tolerance
        if self.risk_tolerance and self.risk_tolerance not in RISK_TOLERANCE_LEVELS:
            errors.append(
                f"Invalid risk_tolerance: {self.risk_tolerance}. Valid: {RISK_TOLERANCE_LEVELS}"
            )
    ...
    def is_complete(self) -> bool:
        ...
        return (
            bool(self.goals) and bool(self.sectors) and self.risk_tolerance is not None
        )
...
class ConversationManager:
    ...
    def __init__(
        ...
        logger.info(
            f"ConversationManager initialized with session_id: {self.session_id}"
        )
    ...
    def set_preferences(
        ...
        new_prefs = UserPreferences(
            ...
            risk_tolerance=(
                ...
                else self._preferences.risk_tolerance
            ),
        )
        ...
        return {
            ...
            "history_summary": {
                ...
                "assistant_messages": sum(
                    1 for m in self._history if m.role == "assistant"
                ),
    ...
    def _should_summarize(self) -> bool:
        ...
        return (
            turn_count > self.max_history_turns
            and turn_count > self._summarized_until_turn
        )
    ...
    def _trigger_summarization(self) -> None:
        ...
        """
        logger.info(
            f"Conversation exceeds {self.max_history_turns} turns. Summarization recommended."
        )
    ...
    def summarize_history_with_llm(self, llm_provider: str = "groq") -> Optional[str]:
        ...
        try:
            ...
            if summary:
                ...
                logger.info(
                    f"History summarized successfully (turn {self._summarized_until_turn})"
                )
...
SUMMARY:"""
    ...
    def import_conversation(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Conversation imported successfully (session: {self.session_id})"
            )
            return True
