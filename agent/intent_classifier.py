...
class ExtractedEntities:
    ...
    def is_empty(self) -> bool:
        ...
        return (
            not self.tickers and
            not self.sectors and
            not self.goals and
            self.risk_tolerance is None and
            self.timeframe is None and
            not self.comparison_items
        )
...
class IntentClassifier:
    ...
    def classify_intent(self, query: str, conversation_context: Optional[List[str]] = None) -> IntentClassification:
        """
        ...
        try:
            ...
            fallback = self._create_fallback_classification(
                query,
                "Unable to understand query. Could you rephrase?"
            )
...
- Only include entities that are explicitly mentioned or strongly implied"""
    ...
    def _parse_llm_response(self, response_text: str, original_query: str) -> IntentClassification:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Unknown intent type: {intent_type_str}, defaulting to needs_clarification")
    ...
    def _apply_fallback_logic(self, classification: IntentClassification) -> IntentClassification:
        """
        ...
        is_greeting = (
            bool( greeting_words.intersection(query_words)) or
            any(phrase in query_lower for phrase in greeting_phrases)
        )
        ...
        if classification.confidence == "low" and not classification.clarification_needed:
...
def classify_intent(query: str, conversation_context: Optional[List[str]] = None) -> IntentClassification:
    """
