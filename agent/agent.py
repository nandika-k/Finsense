...
warnings.filterwarnings(
    ...
    message="unclosed.*<_ProactorBasePipeTransport.*",
)
    ...
    async def _initialize(self):
        ...
        result = await self._send_request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "finsense-coordinator", "version": "1.0.0"},
            },
        )
...
class FinsenseCoordinator:
    ...
    async def fetch_headlines(self, sector: str, timeframe: str) -> Dict[str, Any]:
        ...
        result = await self.news_client.call_tool(
            "fetch_headlines", {"sector": sector, "timeframe": timeframe}
        )
    ...
    async def extract_risk_themes(self, sector: str, timeframe: str) -> Dict[str, Any]:
        ...
        result = await self.news_client.call_tool(
            "extract_risk_themes", {"sector": sector, "timeframe": timeframe}
        )
    ...
    async def identify_sector_risks(self, sector_or_ticker: str) -> Dict[str, Any]:
        ...
        result = await self.news_client.call_tool(
            "identify_sector_risks", {"sector_or_ticker": sector_or_ticker}
        )
        content = result["content"][0]["text"]
    ...
    async def compute_sector_volatility(
        self, sector: str, timeframe: str
    ) -> Dict[str, Any]:
        """Compute volatility for a sector"""
        result = await self.risk_client.call_tool(
            "compute_sector_volatility", {"sector": sector, "timeframe": timeframe}
        )
        content = result["content"][0]["text"]
    ...
    async def compare_sectors(
        self, sector1: str, sector2: str, timeframe: str
    ) -> Dict[str, Any]:
    ...
    async def compute_sector_correlations(
        self, sectors: List[str], timeframe: str
    ) -> Dict[str, Any]:
        """Compute correlations between sectors"""
        result = await self.risk_client.call_tool(
            "compute_sector_correlations", {"sectors": sectors, "timeframe": timeframe}
        )
        content = result["content"][0]["text"]
    ...
    async def conduct_research(
        ...
        for idx, sector in enumerate(sectors, 1):
            ...
            sector_success_count = sum(
                1
                for v in [
                    sector_data["market_performance"],
            ...
            logger.info(
                "  → Sector complete: %d/3 operations successful", sector_success_count
            )
        ...
        if len(sectors) > 1:
            ...
            try:
                async with asyncio.timeout(30.0):
                    ...
                    research_data["portfolio_implications"][
                        "correlations"
                    ] = correlations
                    stats["successful_operations"] += 1
                ...
                logger.error(
                    "✗ Correlation analysis failed: %s: %s", error_type, str(e)[:100]
                )
                research_data["portfolio_implications"]["correlations"] = {
                    "error": str(e),
                    "error_type": error_type,
        ...
        if investment_goals:
            ...
            research_data["goal_based_recommendations"] = self._filter_by_goals(
                research_data["sector_deep_dives"], investment_goals, risk_tolerance
            )
            logger.info(
                "✓ Generated %d goal-aligned sector recommendations",
                len(
                    research_data["goal_based_recommendations"].get(
                        "ranked_sectors", []
                    )
                ),
            )
            ...
            stock_goals = [
                g for g in investment_goals if g in ["esg", "income", "growth"]
            ]
            if stock_goals:
                logger.info(
                    "\n[5/5] Finding stock recommendations for goals: %s",
                    ", ".join(stock_goals),
                )
                top_sector_names = [
                    s["sector"]
                    for s in research_data["goal_based_recommendations"].get(
                        "top_picks", []
                    )[:3]
                ]
                try:
                    async with asyncio.timeout(45.0):
                        stock_recs = await self._recommend_stocks_for_goals(
                            stock_goals, top_sector_names
                        )
                        research_data["stock_recommendations"] = stock_recs
                        total_stocks = sum(
                            len(v.get("stocks", [])) for v in stock_recs.values()
                        )
                        logger.info(
                            "✓ Found %d stock recommendations across %d goals",
                            total_stocks,
                            len(stock_recs),
                        )
                except asyncio.TimeoutError:
    ...
    def _filter_by_goals(
        self, sector_data: Dict[str, Any], goals: List[str], risk_tolerance: str
    ) -> Dict[str, Any]:
        """
        ...
        for sector_name, data in sector_data.items():
            ...
            for goal in goals:
                ...
                if (
                    criteria.get("prioritize") == "high_performance"
                    ...
                    if volatility and volatility <= max_vol:
                        score += 10
                        reasons.append(
                            f"Volatility within {risk_tolerance} risk tolerance"
                        )

                elif (
                    criteria.get("prioritize") == "low_volatility"
                    ...
                    if sector_name in preferred:
                        score += 25
                        reasons.append("ESG-aligned sector")
    ...
    async def _recommend_stocks_for_goals(
        self, goals: List[str], top_sectors: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for goal in goals:
            ...
            if top_sectors:
                ...
                search_sectors = [
                    s for s in top_sectors if s in sectors_to_search
                ] or sectors_to_search[:2]
            else:
            ...
            for sector in search_sectors[:2]:  # Limit to 2 sectors per goal
                try:
                    ...
                    logger.warning(
                        f"Failed to get stock recommendations for {sector}/{goal}: {e}"
                    )
                    continue
